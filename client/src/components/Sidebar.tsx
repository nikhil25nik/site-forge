...
export default function Sidebar({isMenuOpen,project,setProject, isGenerating,setIsGenerating}:SidebarProps){
            ...
            <div className='flex flex-col h-full'>
                        ...
                        if(isMessage){
                            ...
                            return(
                                <div key={msg.id} className={`flex items-start gap-3 ${isUser ? "justify-end":"justify-start"}`}>
                                    ...
                                    <div className={`max-w-[80%] p-2 -x-4 rounded-2xl shadow-sm text-sm mt-5 leading-relaxed ${isUser ? "bg-linear-to-r from-indigo-500 to-indigo-700 text-white rounded-tr-none":"rounded-tl-none bg-gray-800 text-gray-100 "}`}>
                                        {msg.content}
                                    </div>
                                    {isUser && (
                            ...
                            return(
                                ...
                                <div className='flex items-center justify-between'>
                                    {project.current_version_index === ver.id ? (
                                        <button className='px-2 py-1 rounded-md text-xs bg-gray-700 '>Current version</button>
                                    ):(
                                        <button onClick={()=>handleRollBack(ver.id)} className='px-3 py-1 rounded-md text-xs bg-indigo-700 hover:bg-indigo-500 text-white '>Rollback to this version</button>
                                    )}
                    ...
                    {isGenerating && (
                        <div className='flex items-center gap-3 justify-start'>
                            ...
                            <div className='flex gap-1.5 h-full items-end'>
                                ...
                                <span className='size-2 rounded-full animate-bounce bg-gray-600 ' style={{animationDelay:"0.5s"}}/>
                ...
                <form onSubmit={handleRevisions} className='m-3 relative'>
                    <div className='flex items-center gap-2'>
                        <textarea onChange={(e)=>setInput(e.target.value)} placeholder='Describe here....' className='flex-1 p-3 rounded-xl resize-none text-sm outline-none ring ring-gray-700 focus:ring-indigo-500 bg-gray-800 text-gray-100 placeholder-gray-400 transition-all' disabled={isGenerating}/>
                            <button disabled={isGenerating || !input.trim()} className='absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-600 text-white transition-colors disabled:opacity-60'>
                                {
